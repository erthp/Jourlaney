    ...
    public function login(Request $req){
        ...
        if(count($check) >0){
            ...
            if(isset($guideid[0])){
                ...
                $NotificationCount = DB::select("select count(distinct chatRoomId) as notiCount from ChatRoom where ChatRoom.read is null and guideId=".$guideid[0]->guideId);
            ...
            if(isset($touristid[0])){
                ...
                $NotificationCount = DB::select("select count(distinct chatRoomId) as notiCount from ChatRoom where ChatRoom.read is null and touristId=".$touristid[0]->touristId);
                Session::put('NotificationCount', $NotificationCount[0]->notiCount);
            }
            return redirect('/');
